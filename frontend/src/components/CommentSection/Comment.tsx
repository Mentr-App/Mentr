...
const CommentItem: React.FC<CommentItemProps> = ({
    ...
    // Initialize markStatus from comment's user_mark property
    useEffect(() => {
        if (comment.user_mark) {
            setMarkStatus(comment.user_mark);
        }
    }, [comment.user_mark]);

    // Safe way to access localStorage after component mounts
    useEffect(() => {
        if (typeof window !== "undefined") {
            const storedUserId = localStorage.getItem("userId");
            setUserId(storedUserId);

            if (storedUserId && postAuthorId) {
                setIsPostAuthor(storedUserId === postAuthorId);
            }
        }
    }, [postAuthorId]); // when you click one of the buttons:
    const handleMark = async (status: "helpful" | "unhelpful") => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Update the local comment with the user_mark value
    ...
    const handleEditSubmit = async () => {
        if (!isAuthenticated || editText === localComment.content || !editText.trim()) {
            return;
        }
        const token = localStorage.getItem("access_token");
        if (!token) return;
